...
class ProductRepository {
  ...
  Future<Either<FailureModel, PaginationResponse<ProductModel>>> getProducts(
      {int page = 1}) async {
    return _httpService.getData<PaginationResponse<ProductModel>>(
        ListProductParam(page: page));
  }

  Future<Either<FailureModel, PaginationResponse<ProductModel>>> searchProduct(
      String keyword) {
    return _httpService
        .getData<PaginationResponse<ProductModel>>(SearchProductParam(keyword));
  }

  Future<Either<FailureModel, ProductModel?>> searchSingleProduct(
      String keyword) async {
    final result = await _httpService
        .getData<PaginationResponse<ProductModel>>(SearchProductParam(keyword));
    return result.fold(
        (l) => left(l), (r) => right(r.data.isEmpty ? null : r.data.first));
  }

  Future<Either<FailureModel, List<ProductModel>>> addProducts(
      List<ProductModel> products) async {
    return _httpService
        .postData<List<ProductModel>>(CUDProductParam(products: products));
  }

  Future<Either<FailureModel, Unit>> removeProducts(
      List<ProductModel> products) async {
    return _httpService.deleteData(CUDProductParam(products: products));
  }

  Future<Either<FailureModel, Unit>> updateProducts(
      List<ProductModel> products) async {
...
@riverpod
FutureOr<ProductModel?> searchProduct(SearchProductRef ref,
